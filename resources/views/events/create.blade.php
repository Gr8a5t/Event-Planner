...
@section('title', 'Host an Event - EventFlow')

@section('content')
<section class="container" style="max-width: 860px; padding: 4rem 0;">
    <div style="margin-bottom: 2rem;">
        <a href="{{ route('events.index') }}" style="color: var(--text-muted); text-decoration: none; display: inline-flex; align-items: center; gap: 0.4rem; font-size: 0.875rem; font-weight: 600;">
    ...
    <form action="{{ route('events.store') }}" method="POST" id="eventForm">
        ...
        <div id="tab-details" class="tab-panel">
            <div class="card" style="margin-bottom: 1.5rem;">
                <h2 style="font-size: 1.5rem; margin-bottom: 1.5rem; letter-spacing: -0.025em;">Host a New Event</h2>

                <div class="form-group">
                    <label for="title">Event Title</label>
                    <input type="text" name="title" id="title" class="form-control" value="{{ old('title') }}" placeholder="e.g. Annual Tech Symposium" required autofocus>
                    @error('title')<p class="field-error">{{ $message }}</p>@enderror
                </div>

                <div class="form-group">
                    <label for="description">Description</label>
                    <textarea name="description" id="description" class="form-control" rows="5" placeholder="Share what this event is about...">{{ old('description') }}</textarea>
                ...
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 1.5rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="location" id="location" class="form-control" value="{{ old('location') }}" placeholder="e.g. Main Auditorium, Building A" required>
                        @error('location')<p class="field-error">{{ $message }}</p>@enderror
                    </div>
                    <div class="form-group">
                        <label for="capacity">Total Capacity</label>
                        <input type="number" name="capacity" id="capacity" class="form-control" value="{{ old('capacity', 100) }}" min="1" required>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; align-items: start;">
                ...
                <div class="card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" name="cover_image" id="cover_image" class="form-control" value="{{ old('cover_image') }}" placeholder="https://images.unsplash.com/...">
                    ...
                    <div class="form-group">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 0.5rem; margin-bottom: 1rem;">
                            ...
                            @foreach($presets as $url)
                                <div class="bg-preset" style="background-image: url('{{ $url }}');" data-url="{{ $url }}"></div>
                            @endforeach
                        </div>
                        <input type="url" name="background_image" id="background_image" class="form-control" value="{{ old('background_image') }}" placeholder="Or paste custom background URL...">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                        <div class="form-group">
                            ...
                            <div style="display: flex; gap: 0.5rem; align-items: center;">
                                <input type="color" name="theme_color" id="theme_color" value="{{ old('theme_color', '#ffffff') }}" style="width:44px; height:44px; border-radius:10px; border:1.5px solid #e5e7eb; cursor:pointer; padding:2px;">
                                <input type="text" id="theme_color_hex" class="form-control" value="{{ old('theme_color', '#ffffff') }}" style="font-family: monospace;" maxlength="7">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="font_family" id="font_family" class="form-control">
                                @foreach(['Outfit' => 'Outfit (Default)', 'Inter' => 'Inter — Clean & Modern', 'Playfair Display' => 'Playfair Display — Elegant', 'Space Grotesk' => 'Space Grotesk — Techy', 'Syne' => 'Syne — Bold & Artistic', 'Lato' => 'Lato — Friendly', 'Merriweather' => 'Merriweather — Classic Serif'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('font_family', 'Outfit') === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div id="preview-card" style="border-radius: 20px; overflow: hidden; border: 1.5px solid var(--border); box-shadow: 0 10px 40px rgba(0,0,0,0.08);">
                        <div id="preview-cover" style="height: 140px; background: #e5e7eb; background-size: cover; background-position: center; display: flex; align-items: flex-end; padding: 1rem;">
                            <span id="preview-badge" style="background:#000; color:#fff; font-size:0.7rem; font-weight:700; padding:0.2rem 0.6rem; border-radius:99px; text-transform:uppercase;">Upcoming</span>
                        </div>
                        <div id="preview-body" style="padding: 1.25rem; background: #fff;">
                            <p id="preview-title" style="font-size: 1.1rem; font-weight: 800; margin-bottom: 0.4rem; letter-spacing: -0.02em;">Your Event Title</p>
                            <p style="font-size: 0.8rem; color: #6b7280;">📍 Location  •  🕐 Time</p>
                            <div id="preview-btn" style="margin-top: 1rem; display: inline-block; background: #000; color: #fff; font-size: 0.75rem; font-weight: 700; padding: 0.5rem 1.25rem; border-radius: 99px;">Register Now</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div style="display: flex; gap: 1rem; justify-content: flex-end; margin-top: 2rem;">
            ...
            <button type="submit" class="btn btn-primary" style="padding: 0.8rem 2.5rem;">Launch Event →</button>
        </div>
    </form>
</section>
@endsection
...
<script>
    ...
    // Helper to update background preview
